...
## 4. Reduce Default Scale (Faster rendering)
...
```typescript
/**
 * Scale of a page when rendering to image. Defaults to 1.5.
 * - 1.0: Fast, suitable for thumbnails (~72 DPI)
 * - 1.5: Balanced, good for screen display (~108 DPI)
 * - 2.0: High quality, good for printing (~144 DPI)
 * - 3.0+: Very high quality, large files
 * @default 1.5
 */
scale?: number;
...
```typescript
// Balanced quality/speed (new default)
const images = await pdf2image(pdfPath);
...
const printImages = await pdf2image(pdfPath, { scale: 3.0 });
```
...
| Improvement           | Impact         | Complexity | Breaking Change      |
| --------------------- | -------------- | ---------- | -------------------- |
| Auto-tune Concurrency | 3-4x speedup   | Low        | No (better default)  |
| Canvas Pooling        | 15-20% speedup | Medium     | No                   |
| JPEG Default          | 20-30% speedup | Low        | Yes (output format)  |
| Reduce Scale          | 30-50% speedup | Low        | Yes (output quality) |
| Stream Processing     | Memory savings | Medium     | No (new API)         |
